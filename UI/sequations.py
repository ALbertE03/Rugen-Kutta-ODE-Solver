...
if option == "2x2":
    ...
    A = np.array(matrix)
    Y0 = np.array([col1.number_input(f"Valor inicial de x{i+1}", format="%.2f", key=f"Y0_{i}") for i in range(2)]).reshape(2, 1)
    ...
    if col1.button("Graficar"):
        ...
        res1.table(np.vectorize(lambda x: f"{x:.2e}" if abs(x) >= 1e7 or abs(x) < 1e-7 else x)(exp_At))
        ...
        eigenvalues_display = [f"{val.real:.3f}".rstrip('0').rstrip('.') + (f" + {val.imag:.3f}i".rstrip('0').rstrip('.') if val.imag != 0 else '') for val in eigenvalues]
        res3.table(eigenvalues_display)
        res4.markdown("#### Vectores propios:")
        eigenvectors_display = [[f"{val.real:.3f}".rstrip('0').rstrip('.') + (f" + {val.imag:.3f}i".rstrip('0').rstrip('.') if val.imag != 0 else '') for val in row] for row in eigenvectors.T]
